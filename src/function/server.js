...
export function CheckProfileDone(student) {
  ...
  const allPresentationsComplete = presentations.every(
    (p) => p.course && p.bookId && p.bookName && p.Video && p.Img && p.Comment,
  );
...
export async function CheckSlide(url) {
  ...
  if (!googleSlidesRegex.test(url)) {
    return {
      isValid: false,
      message: "Link Slide không đúng định dạng của Google Slides.",
    };
  }
  ...
  try {
    ...
    if (response.url.includes("accounts.google.com")) {
      return {
        isValid: false,
        message: "Link Google Slides phải được chia sẻ công khai (public).",
      };
    }
    if (!response.ok) {
      return {
        isValid: false,
        message: `Link Slide không truy cập được, status code: ${response.status}.`,
      };
    }
    return { isValid: true };
  } catch (error) {
    ...
    return {
      isValid: false,
      message: "Không thể xác thực được URL của Slide do lỗi mạng.",
    };
  }
}
