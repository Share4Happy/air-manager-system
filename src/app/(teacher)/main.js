    ...
    const renderTimePeriodSelector = () => {
        ...
        switch (timeGranularity) {
            case 'week':
                ...
                return (
                    <div className="flex-1 flex gap-2">
                        <Menu isOpen={isWeekMenuOpen} onOpenChange={setIsWeekMenuOpen} customButton={<div className='input text-sm font-normal text-[var(--text-primary)]'>{weekButtonText}</div>} menuItems={
                            <div className="p-2 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded mt-1">
            ...
            case 'month':
                return (
                    <div className="flex-1 flex gap-2">
                        <Menu isOpen={isMonthMenuOpen} onOpenChange={setIsMonthMenuOpen} customButton={<div className='input text-sm font-normal text-[var(--text-primary)]'>{`Tháng ${month + 1}`}</div>} menuItems={<div className="p-2 mt-1 rounded bg-[var(--bg-primary)] border border-[var(--border-color)] flex flex-col z-10">{Array.from({ length: 12 }, (_, i) => i).map(m => <p key={m} className='text-sm font-normal text-[var(--text-primary)]' onClick={() => { setSelectedDate(new Date(year, m, 1)); setIsMonthMenuOpen(false); }}>{`Tháng ${m + 1}`}</p>)}</div>} />
                        <Menu isOpen={isYearMenuOpen} onOpenChange={setIsYearMenuOpen} customButton={<div className='input text-sm font-normal text-[var(--text-primary)]'>{`Năm ${year}`}</div>} menuItems={<div className="p-2 mt-1 rounded bg-[var(--bg-primary)] border border-[var(--border-color)] flex flex-col z-10">{yearOptions.map(y => <p key={y} className='text-sm font-normal text-[var(--text-primary)]' onClick={() => { setSelectedDate(new Date(y, month, 1)); setIsYearMenuOpen(false); }}>{`Năm ${y}`}</p>)}</div>} />
                    </div>
                );
            case 'quarter':
                return (
                    <div className="flex-1 flex gap-2">
                        <Menu isOpen={isQuarterMenuOpen} onOpenChange={setIsQuarterMenuOpen} customButton={<div className='input text-sm font-normal text-[var(--text-primary)]'>{`Quý ${quarter + 1}`}</div>} menuItems={<div className="p-2 mt-1 rounded bg-[var(--bg-primary)] border border-[var(--border-color)] flex flex-col z-10">{[0, 1, 2, 3].map(q => <p key={q} className='text-sm font-normal text-[var(--text-primary)]' onClick={() => { setSelectedDate(new Date(year, q * 3, 1)); setIsQuarterMenuOpen(false); }}>{`Quý ${q + 1}`}</p>)}</div>} />
    ...
    return (
        <div className="flex gap-2">
            <div className="w-[350px] shrink-0 flex flex-col gap-2">
                <p className='text-base font-semibold text-[var(--text-primary)]'>Bảng điều khiển</p>
                <p className='text-sm font-normal text-[var(--text-primary)]' style={{ marginTop: '-5px', marginBottom: '10px' }}>Xin chào, {currentUser.name}!</p>

                <div className="flex gap-2 items-stretch">
                    <div className="flex-1 flex cursor-pointer">
                        <Menu isOpen={isGranularityMenuOpen} onOpenChange={setIsGranularityMenuOpen} customButton={<div className='input text-sm font-normal text-[var(--text-primary)]'>{granularityText[timeGranularity]}</div>} menuItems={
                            <div className="p-2 mt-1 rounded bg-[var(--bg-primary)] border border-[var(--border-color)] flex flex-col z-10">
                                {Object.entries(granularityText).map(([key, value]) => (
                                    <p key={key} className='text-sm font-normal text-[var(--text-primary)]' onClick={() => { setTimeGranularity(key); setIsGranularityMenuOpen(false); }}>{value}</p>
                                ))}
                            </div>}
                        />
                    </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                    <div className="flex flex-col items-center justify-center p-3 bg-[var(--bg-primary)] rounded border border-[var(--border-color)] text-center gap-1"><span>Tổng buổi dạy</span><strong>{personalStats.totalWorkload}</strong></div>
                    <div className="flex flex-col items-center justify-center p-3 bg-[var(--bg-primary)] rounded border border-[var(--border-color)] text-center gap-1"><span>Lớp chủ nhiệm</span><strong>{personalStats.homeroomClasses.length}</strong></div>
                    <div className="flex flex-col items-center justify-center p-3 bg-[var(--bg-primary)] rounded border border-[var(--border-color)] text-center gap-1"><span>Tổng vi phạm</span><strong className="text-red-500">{personalStats.violations.total}</strong></div>
                </div>

                <div className="flex flex-col bg-[var(--bg-primary)] p-4 rounded border border-[var(--border-color)]">
                    <p className='text-sm font-semibold text-[var(--text-primary)]' style={{ paddingBottom: 8, marginBottom: 3, borderBottom: 'thin dashed var(--border-color)' }}>Lịch dạy sắp tới</p>
                    {personalStats.upcomingClasses.length > 0 ? personalStats.upcomingClasses.slice(0, 5).map((lesson, i) =>
                        <div key={i} className="flex justify-between items-center text-sm p-2 transition-all duration-200 bg-[var(--bg-primary)] rounded">
                            <span>{lesson.courseName}</span>
                            <strong>{new Date(lesson.Day).toLocaleDateString('vi-VN')}</strong>
                        </div>
                    ) : <p className="text-sm text-gray-400 text-center py-2.5">Không có lịch dạy mới.</p>}
                </div>
                ...
                <div className="flex flex-col bg-[var(--bg-primary)] p-4 rounded border border-[var(--border-color)]">
                    ...
                    {personalStats.homeroomClasses.length > 0 ? personalStats.homeroomClasses.map((course, i) =>
                        <Link href={`/course/${course.ID || course._id}`} key={i} className="flex justify-between items-center text-sm p-2 transition-all duration-200 bg-[var(--bg-primary)] rounded">
                            ...
                            <p className='text-sm font-normal text-[var(--text-primary)]'>{course.Student?.length || 0} Học viên</p>
                        </Link>
                    ) : <p className="text-sm text-gray-400 text-center py-2.5">Bạn chưa chủ nhiệm lớp nào.</p>}
                </div>
            </div>

            <div className="flex-1 flex flex-col bg-[var(--bg-primary)] p-4 rounded border border-[var(--border-color)]">
                <p className='text-xs font-medium text-[var(--text-primary)]'>Thống kê khối lượng công việc</p>
